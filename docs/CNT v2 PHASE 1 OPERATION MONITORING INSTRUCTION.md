...
```text
목표 = 데이터 누적 + 이상 징후 자동 감지
```

현재 시스템은 정상 동작 중이므로
**개입 없이 감시만 하는 구조**를 만든다.
...
```text
consecutive_losses >= 3 → 정상 (cooldown 진입 확인)
consecutive_losses >= 5 → 위험
```

---

### 2.5 exit failsafe 작동 여부
